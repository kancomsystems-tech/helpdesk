...
import { CommentIcon, EmailIcon, ReplyAllIcon, ReplyIcon } from "@/components/icons/";
...
function splitIfString(str?: string | string[]) {
  ...
  return str.map((item) => item.trim()).filter(Boolean);
}
...
function replyToEmail(data: object) {
  ...
  emailEditorRef.value.addToReply(
    ...
    {
      from: data.from,
      to: splitIfString(data.originalTo),
      cc: splitIfString(data.originalCc),
